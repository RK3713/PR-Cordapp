...
public class StudentInfoBODeserializer extends JsonDeserializer<StudentInfoBO> {
    ...
    public StudentInfoBO deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException, JsonProcessingException {
        ...
        if (node.get("degreeDetails") != null) {
            ...
            degreeDetailsBO.setDegreeName(degreeDetails.get("passingYear") != null ? degreeDetails.get("passingYear").asText() : null);
            degreeDetailsBO.setDegreeName(degreeDetails.get("passingDivision") != null ? degreeDetails.get("passingDivision").asText() : null);
            degreeDetailsBO.setDegreeName(degreeDetails.get("fullName") != null ? degreeDetails.get("fullName").asText() : null);
            degreeDetailsBO.setDegreeName(degreeDetails.get("fatherName") != null ? degreeDetails.get("fatherName").asText() : null);
            degreeDetailsBO.setDegreeName(degreeDetails.get("specializationField") != null ? degreeDetails.get("specializationField").asText() : null);
        ...
        if (node.get("transcipt") != null) {
            ...
            if (transcript.get("semester") != null) {
                JsonNode jsonArrayNode = node.get("semester");
                        ...
                        if (objNode.get("subject") != null) {
                            ...
                            if (jsonArrayNode1.isArray()) {
                                for (JsonNode jsonNode : jsonArrayNode1) {
                                    ...
                                    subjectBO.setPassed(jsonNode.get("isPassed") != null ? jsonNode.get("isPassed").asBoolean() : null);
