...
public class WesController extends CommonController {
    ...
    public ResponseEntity respondToPrRequest(@RequestParam(value ="requestId",required = true) String requestId, @RequestBody PRBO prbo) throws Exception{
        ...
        logger.info("I m at initialization");
        ...
        logger.info("previousPrState "+ previousPrState);
            ...
            logger.info("I m after first if condition");

            try {
                newPRState = convertToPRStateforUpdate(previousPrState.get(0).getState().getData(),prbo);
